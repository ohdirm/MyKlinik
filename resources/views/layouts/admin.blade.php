        ...
        >
            <div class="w-64 flex flex-col h-full shrink-0">
                ...
                <nav class="flex-1 px-4 py-4 space-y-1 overflow-y-auto !bg-transparent !border-none">
                    ...
                    <a href="{{ route('admin.archive.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition {{ request()->routeIs('admin.archive.*') ? 'bg-white/15 text-white' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
        ...
        <div class="flex-1 flex flex-col overflow-hidden bg-gray-50 dark:bg-[#141b18]">
            ...
            <header class="bg-white dark:bg-[#1c2622] border-b border-[#e2efe7] dark:border-[#283731] h-20 flex items-center justify-between px-6 shrink-0 transition-all duration-300">
                <div class="flex items-center gap-4">
                    <button @click="sidebarOpen = !sidebarOpen" class="p-2 rounded-lg hover:bg-brand-light dark:hover:bg-gray-700 transition-colors focus:outline-none" aria-label="Toggle Navigation">
                        <svg class="w-6 h-6 text-brand dark:text-blue-300 transition-transform duration-300" :class="sidebarOpen ? '' : 'rotate-180'" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
