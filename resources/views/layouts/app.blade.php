...
<body class="min-h-screen font-sans flex flex-col transition-colors duration-300" x-data="{ darkMode: document.documentElement.classList.contains('dark') }">
    ...
    {{-- Navbar --}}
    <nav class="bg-white/80 dark:bg-[#0a0f0d]/85 text-gray-900 dark:text-white shadow-sm sticky top-0 z-40 backdrop-blur-md border-b border-gray-100 dark:border-[#1c2622] transition-colors duration-300" x-data="{ open: false }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="hidden md:flex items-center gap-1">
                    <a href="{{ url('/') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all active:scale-95 {{ request()->is('/') ? 'bg-brand/10 text-brand' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#1c2622] hover:text-brand dark:hover:text-white' }}">Beranda</a>
                    @auth
                        @if(Auth::user()->isPatient())
                            <a href="{{ route('booking.index') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all active:scale-95 {{ request()->is('booking') ? 'bg-brand/10 text-brand' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#1c2622] hover:text-brand dark:hover:text-white' }}">Booking</a>
                            <a href="{{ route('status-dokter') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all active:scale-95 {{ request()->is('status-dokter') ? 'bg-brand/10 text-brand' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#1c2622] hover:text-brand dark:hover:text-white' }}">Status Dokter</a>
                            <a href="{{ route('patient.dashboard') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all active:scale-95 {{ request()->is('antrean-saya') ? 'bg-brand/10 text-brand' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#1c2622] hover:text-brand dark:hover:text-white' }}">Antrean Saya</a>
                            <a href="{{ route('profile.index') }}" class="px-4 py-2 rounded-xl text-sm font-semibold transition-all active:scale-95 {{ request()->is('profile') ? 'bg-brand/10 text-brand' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#1c2622] hover:text-brand dark:hover:text-white' }}">Profile</a>
                        @endif
                    @endauth

                    {{-- Auth buttons --}}
                    @guest
                        <a href="{{ route('login') }}" class="ml-2 px-4 py-2 rounded-xl text-sm font-semibold text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-[#1c2622] transition-all">Masuk</a>
                    ...
                    @else
                        ...
                        <div class="relative ml-2" x-data="notificationBell()" x-init="fetchNotifications(); setInterval(() => fetchNotifications(), 30000)">
                            <button @click="open = !open; if(open) fetchNotifications()" class="relative p-2.5 rounded-xl hover:bg-gray-100 dark:hover:bg-[#1c2622] text-gray-500 dark:text-gray-400 hover:text-brand dark:hover:text-white transition-all cursor-pointer" aria-label="Notifikasi">
                            ...
                            <div x-show="open" @click.away="open = false"
                                 ...
                                 class="absolute right-0 mt-2 w-96 bg-white dark:bg-[#141b18] rounded-2xl shadow-2xl border border-gray-100 dark:border-[#283731] z-50 overflow-hidden" style="display: none;">

                                {{-- Header --}}
                                <div class="px-5 py-3.5 border-b border-gray-100 dark:border-[#1c2622] flex items-center justify-between bg-gray-50/80 dark:bg-[#0f1714]/80">
                                ...
                                {{-- List --}}
                                <div class="max-h-80 overflow-y-auto divide-y divide-gray-50 dark:divide-[#1c2622]">
                                    <template x-if="notifications.length === 0">
                                        <div class="px-5 py-10 text-center">
                                            <div class="w-14 h-14 bg-gray-100 dark:bg-[#1c2622] rounded-full flex items-center justify-center mx-auto mb-3">
                                                <svg class="w-7 h-7 text-gray-300 dark:text-[#283731]" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0"/></svg>
                                            </div>
                                    ...
                                    <template x-for="notif in notifications" :key="notif.id">
                                        <div @click="markAsRead(notif.id); notif.read = true"
                                             class="px-5 py-3.5 hover:bg-gray-50 dark:hover:bg-[#1c2622]/50 cursor-pointer transition-colors relative"
                                ...
                                {{-- Footer --}}
                                <div x-show="notifications.length > 0" class="px-5 py-3 border-t border-gray-100 dark:border-[#1c2622] bg-gray-50/80 dark:bg-[#0f1714]/80 text-center" style="display: none;">
                        ...
                        <div class="relative ml-2" x-data="{ dropdown: false }">
                            ...
                            <div x-show="dropdown" @click.away="dropdown = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" class="absolute right-0 mt-2 w-52 bg-white dark:bg-[#141b18] rounded-2xl shadow-xl py-2 z-50 border border-gray-100 dark:border-[#1c2622]">
                                @if(Auth::user()->isPatient())
                                    <a href="{{ route('patient.dashboard') }}" class="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-[#1c2622] transition-colors">
                                    ...
                                    <a href="{{ route('profile.index') }}" class="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-[#1c2622] transition-colors">
                                ...
                                @endif
                                <div class="border-t border-gray-100 dark:border-[#1c2622] my-1"></div>
                ...
                {{-- Mobile hamburger --}}
                <button @click="open = !open" class="md:hidden p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-[#1c2622] text-gray-600 dark:text-gray-300 transition-all" aria-label="Menu">
        ...
        {{-- Mobile menu --}}
        <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="md:hidden border-t border-gray-100 dark:border-[#1c2622] bg-white/95 dark:bg-[#0a0f0d]/95 backdrop-blur-md">
            <div class="px-4 py-4 space-y-2">
                ...
                @auth
                    @if(Auth::user()->isPatient())
                        ...
                        <a href="{{ route('profile.index') }}" class="block px-4 py-2.5 rounded-xl text-sm font-semibold text-gray-700 dark:text-gray-200 hover:bg-brand/5 hover:text-brand">Profile</a>
                    @endif
                    <div class="border-t border-gray-100 dark:border-[#1c2622] mt-2 pt-2">
    ...
    {{-- Footer --}}
    <footer class="bg-[#1B2621] dark:bg-[#0f1714] text-gray-300 dark:text-gray-400 border-t border-[#2d3e35] dark:border-[#1c2622]">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div class="border-t border-[#2d3e35] dark:border-[#1c2622] mt-10 pt-8 text-center text-xs opacity-50">
