...
@section('title', 'Edit Jadwal - Klinik App')
@section('header_title', 'Edit Jadwal Praktik')
...
<div class="max-w-2xl bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
    <form action="{{ route('schedules.update', $schedule) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <select id="doctor_id" name="doctor_id" required
                ...
                @foreach($doctors as $doctor)
                    <option value="{{ $doctor->id }}" {{ old('doctor_id', $schedule->doctor_id) == $doctor->id ? 'selected' : '' }}>{{ $doctor->name }} ({{ $doctor->clinic->name }})</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="number" id="max_patients" name="max_patients" value="{{ old('max_patients', $schedule->max_patients) }}" min="1" required
                class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-indigo-100 focus:border-indigo-400 outline-none transition-all">
