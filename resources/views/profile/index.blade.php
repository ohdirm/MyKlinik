...
@section('content')
<div class="min-h-screen bg-gradient-to-br from-[#f2faf5] via-white to-[#e8f5ed] dark:from-[#141b18] dark:via-[#0a0f0d] dark:to-[#141b18] py-10 transition-colors" x-data="profilePage()">
    <div class="max-w-2xl mx-auto px-4">
        {{-- Back --}}
        <div class="mb-6">
            <a href="{{ url('/') }}" class="inline-flex items-center gap-2 group text-sm font-medium text-gray-500 dark:text-gray-400">
        ...
        {{-- Alerts --}}
        @if(session('warning'))
        <div class="mb-6 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 text-amber-800 dark:text-amber-300 px-5 py-4 rounded-2xl text-sm flex items-center gap-3">
            <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z"/></svg>
            {{ session('warning') }}
        </div>
        @endif
        @if(session('success'))
        <div class="mb-6 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 px-5 py-4 rounded-2xl text-sm flex items-center gap-3">
        ...
        <div class="flex gap-2 mb-6">
            <button @click="tab = 'profile'" :class="tab === 'profile' ? 'bg-brand text-white shadow-md' : 'bg-white dark:bg-[#1c2622] text-gray-600 dark:text-gray-300 border border-[#e2efe7] dark:border-[#283731]'" class="flex-1 py-3 rounded-2xl text-sm font-semibold transition-all flex items-center justify-center gap-2 cursor-pointer">
            ...
            <button @click="tab = 'family'" :class="tab === 'family' ? 'bg-brand text-white shadow-md' : 'bg-white dark:bg-[#1c2622] text-gray-600 dark:text-gray-300 border border-[#e2efe7] dark:border-[#283731]'" class="flex-1 py-3 rounded-2xl text-sm font-semibold transition-all flex items-center justify-center gap-2 cursor-pointer">
