...
@section('title', 'Tambah Staff Baru — MyKlinik911')
@section('page_title', 'Tambah Staff Baru')
@section('page_subtitle', 'Daftarkan akun pekerja klinik')
...
<div class="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div class="bg-white dark:bg-[#1c2622] rounded-[2.5rem] shadow-xl border border-gray-100 dark:border-gray-800 overflow-hidden">
        ...
        <div class="px-10 py-8 bg-gray-50/50 dark:bg-gray-800/10 border-b border-gray-100 dark:border-gray-800">
            <h3 class="text-xl font-black text-gray-900 dark:text-white tracking-tight">Informasi Akun Staff</h3>
            <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Gunakan email yang valid untuk proses masuk.</p>
        </div>

        <form action="{{ route('admin.staff.store') }}" method="POST" class="p-10 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                {{-- Password --}}
                <div class="space-y-2">
                    <label for="password" class="text-sm font-bold text-gray-800 dark:text-gray-200 px-1">Password</label>
