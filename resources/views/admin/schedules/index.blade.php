...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">Kelola jadwal praktik per dokter</p>
    </div>
...
<div class="space-y-4">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="flex items-center justify-between px-5 py-4 bg-gray-50 border-b border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="font-semibold text-gray-900 text-sm">{{ $doctor->name }}</p>
                    <p class="text-xs text-teal-600">{{ $doctor->specialization_label }}</p>
                </div>
            </div>
            <span class="text-xs text-gray-400 font-medium">{{ $doctor->schedules->count() }} jadwal aktif</span>
        </div>

        {{-- Schedules --}}
        @if($doctor->schedules->isEmpty())
            <div class="px-5 py-6 text-center text-sm text-gray-400">
                Belum ada jadwal. <a href="{{ route('admin.schedules.create') }}?doctor={{ $doctor->id }}" class="text-brand hover:underline">Tambah sekarang</a>
            </div>
        @else
            <div class="divide-y divide-gray-50">
                ...
                <div class="flex items-center justify-between px-5 py-3 hover:bg-gray-50 transition-colors">
                    <div class="flex items-center gap-4">
                        {{-- Day badge --}}
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border border-gray-200 bg-white text-gray-700 min-w-[75px] justify-center shadow-sm">
                    ...
                    <div class="flex items-center gap-2">
                        <a href="{{ route('admin.schedules.edit', $s) }}"
                           class="inline-flex items-center gap-1 rounded-lg border border-brand/20 bg-brand/10 px-3 py-1 text-xs font-semibold text-brand hover:bg-brand/20 transition">
                        ...
                        <form method="POST" action="{{ route('admin.schedules.destroy', $s) }}" onsubmit="return confirm('Hapus jadwal {{ $dayNames[$s->day_of_week] }} untuk {{ $doctor->name }}?')" class="inline">
                            @csrf @method('DELETE')
                            <button type="submit" class="inline-flex items-center gap-1 rounded-lg border border-red-200 bg-red-50 px-3 py-1 text-xs font-semibold text-red-600 hover:bg-red-100 transition">
    ...
    @empty
        <div class="bg-white rounded-xl shadow-sm p-12 text-center text-gray-400">
            ...
            <p class="font-medium">Belum ada dokter aktif</p>
