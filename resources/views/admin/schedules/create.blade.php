...
@section('title', 'Tambah Jadwal — MyKlinik911')
@section('content')
<h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Tambah Jadwal</h1>
<div class="bg-white dark:bg-[#1c2622] border border-[#e2efe7] dark:border-[#283731] rounded-xl shadow-sm p-6 max-w-2xl transition-colors duration-200">
    ...
    <form method="POST" action="{{ route('admin.schedules.store') }}">
        ...
        <div class="space-y-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Dokter</label>
            ...
            <div>
                ...
                <select name="day_of_week" class="input-base" required>
                    @foreach(['0'=>'Minggu','1'=>'Senin','2'=>'Selasa','3'=>'Rabu','4'=>'Kamis','5'=>'Jumat','6'=>'Sabtu'] as $v=>$l)
                        <option value="{{ $v }}" {{ old('day_of_week') === $v ? 'selected' : '' }}>{{ $l }}</option>
                    @endforeach
                </select>
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="time" name="start_time" class="input-base" value="{{ old('start_time', '08:00') }}" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Jam Selesai</label>
                    <input type="time" name="end_time" class="input-base" value="{{ old('end_time', '12:00') }}" required>
                </div>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Maks Pasien</label>
                <input type="number" name="max_patients" class="input-base" value="{{ old('max_patients', 20) }}" min="1" required>
            </div>
