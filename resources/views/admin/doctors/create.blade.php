...
<div class="bg-white dark:bg-[#1c2622] border border-[#e2efe7] dark:border-[#283731] rounded-xl shadow-sm p-6 max-w-2xl transition-colors duration-200">
    ...
    <form method="POST" action="{{ route('admin.doctors.store') }}" enctype="multipart/form-data">
        ...
        <div class="space-y-4">
            ...
            <div x-data="specializationPicker('{{ old('specialization') }}')" x-init="init()">
                ...
                <div class="flex gap-2 items-center">
                    <select name="specialization" x-ref="selectEl" class="input-base flex-1" required>
                        ...
                        @foreach($specializations as $spec)
                            <option value="{{ $spec->value }}" {{ old('specialization') == $spec->value ? 'selected' : '' }}>
            ...
            <div class="flex items-center gap-2">
                ...
                <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-brand focus:ring-brand focus:ring-offset-white dark:focus:ring-offset-gray-900" {{ old('is_active', true) ? 'checked' : '' }}>
...
<script>
function specializationPicker(oldValue) {
    return {
        showModal: false,
        newLabel: '',
        saving: false,
        ...
        init() {
            ...
            if (oldValue && this.$refs.selectEl) {
                this.$refs.selectEl.value = oldValue;
            }
        },
