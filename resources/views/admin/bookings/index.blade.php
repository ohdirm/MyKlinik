    ...
    <form method="GET" class="flex gap-4 flex-wrap items-end">
        ...
        <div>
            ...
            <select name="status" class="input-base">
                <option value="">Semua</option>
                @foreach(['PENDING','CONFIRMED','EXAMINING','REJECTED','DONE','CANCELLED', 'EXPIRED'] as $s)
                ...
                <tr class="hover:bg-[#F6FBF8] dark:hover:bg-[#1c2622]/50" id="row-{{ $b->id }}">
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-2" id="actions-{{ $b->id }}">
                            ...
                            @if($b->status === 'PENDING')
                                <button onclick="confirmBooking({{ $b->id }}, this)" class="inline-flex items-center gap-1.5 text-xs font-semibold bg-green-500 hover:bg-green-600 text-white px-3.5 py-2 rounded-lg transition cursor-pointer shadow-sm">
                                ...
                                <button onclick="openRejectModal({{ $b->id }})" class="inline-flex items-center gap-1.5 text-xs font-semibold bg-red-500 hover:bg-red-600 text-white px-3.5 py-2 rounded-lg transition cursor-pointer shadow-sm">
                            ...
                            @elseif($b->status === 'CONFIRMED')
                                @php $isBusy = in_array($b->doctor_id, $busyDoctorIds); @endphp
                                <button onclick="examineBooking({{ $b->id }})" 
                                        @if($isBusy) disabled title="Dokter sedang melayani pasien lain" @endif
                                        class="inline-flex items-center gap-1.5 text-xs font-semibold {{ $isBusy ? 'bg-gray-400 cursor-not-allowed opacity-75' : 'bg-blue-500 hover:bg-blue-600 cursor-pointer' }} text-white px-3.5 py-2 rounded-lg transition shadow-sm">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z"/></svg>
                                    {{ $isBusy ? 'Dokter Sibuk' : 'Periksa Pasien' }}
                                </button>
                            @elseif($b->status === 'EXAMINING')
                                <button onclick="doneBooking({{ $b->id }})" class="inline-flex items-center gap-1.5 text-xs font-semibold bg-emerald-500 hover:bg-emerald-600 text-white px-3.5 py-2 rounded-lg transition cursor-pointer shadow-sm animate-pulse">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5"/></svg>
                                    Selesai Periksa
                                </button>
                            @endif
                            ...
                            <div class="relative" x-data="{ menuOpen: false }">
                                ...
                                <div x-show="menuOpen" @click.away="menuOpen = false" x-transition
                                    ...
                                    @if(in_array($b->status, ['CONFIRMED', 'EXAMINING']))
                                    <a href="{{ $b->whatsapp_link }}" target="_blank" @click="menuOpen = false"
                                       class="w-full text-left px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300 flex items-center gap-2.5 cursor-pointer transition">
...
// Helper: open detail modal from dynamic HTML (outside Alpine scope)
function openDetailById(id) {
    // Dispatch a custom event that the Alpine component can listen to
    const el = document.querySelector('[x-data]');
    if (el && el.__x) {
        const data = el.__x.$data;
        data.selected = data.bookings.find(b => b.id === id);
...
function examineBooking(id) {
    if (!confirm('Mulai periksa pasien ini?\n\nStatus dokter akan otomatis berubah menjadi "Sedang Melayani Pasien".')) return;
    fetch(`/admin/bookings/${id}/examine`, {method:'PATCH',headers:{'X-CSRF-TOKEN':csrf,'Accept':'application/json'}})
        .then(r => {
            if (!r.ok) return r.json().then(d => { throw new Error(d.message || 'Server error ' + r.status); });
            return r.json();
        })
        .then(data => {
            if(data.success) {
...
function doneBooking(id) {
    if (!confirm('Selesaikan pemeriksaan pasien ini?\n\nStatus dokter akan otomatis kembali ke "Tersedia".')) return;
    fetch(`/admin/bookings/${id}/done`, {method:'PATCH',headers:{'X-CSRF-TOKEN':csrf,'Accept':'application/json'}})
        ...
        .then(data => {
            if(data.success) {
                updateRow(id,'DONE','bg-gray-100 text-gray-800');
                alert('Pemeriksaan selesai. Status dokter kembali tersedia.');
            }
        })
        .catch(e => { alert('Gagal: ' + e.message); console.error(e); });
}
...
function updateRow(id, status, badgeClass, waLink = null, startTime = null) {
    ...
    // Build primary button based on status
    let primaryBtn = '';
    let showWa = false;

    if(status==='CONFIRMED') {
        primaryBtn = `<button onclick="examineBooking(${id})" class="inline-flex items-center gap-1.5 text-xs font-semibold bg-blue-500 hover:bg-blue-600 text-white px-3.5 py-2 rounded-lg transition cursor-pointer shadow-sm">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z"/></svg>
            Periksa Pasien</button>`;
        showWa = true;
    }
    else if(status==='EXAMINING') {
        primaryBtn = `<button onclick="doneBooking(${id})" class="inline-flex items-center gap-1.5 text-xs font-semibold bg-emerald-500 hover:bg-emerald-600 text-white px-3.5 py-2 rounded-lg transition cursor-pointer shadow-sm animate-pulse">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5"/></svg>
            Selesai Periksa</button>`;
        showWa = true;
    }
    ...
    actions.innerHTML = primaryBtn + dropdown;
}
